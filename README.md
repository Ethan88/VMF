...
* New fund manager can use the CLI to instantiate a new wallet address on the liquid-tier chain (nickname: Lisa chain) which is shared by all funds
  * Next, the fund manager must enter his information. This Lisa address will turn into a Fund Manager object containing attributes: bitcoin address, ether address, name, company, URL, keybase key, address, phone, email, user@handle, SSN/EIN, and image URL
...
###Request permission from i2 to start a fund
Now with a Lisa-chain wallet address, fund manager may send a zero-coin transaction to Iterative Instinct's Lisa address, requesting to start a fund.
  ...
  * If approved by Iterative Instinct human operators, our servers generate one (1) new A-tier and one (1) new R-tier chain to serve as seed peers. Next, the fund manager's Docker instance connects to these new chains, and generates A-tier and R-tier addresses, associating them with the L-tier wallet address that sent the request. These addresses are now all part of the Fund Manager object.
    * Return "successfully started your A-tier and R-tier chains!"
    * Return fund manager's new A-chain and R-chain wallet addresses, and remind him his Lisa address, so he knows them all
...
* To invite a new member-investor to the system, use the CLI to create a new user associated with the person's phone number (eg., user 2123002020)
 ...
 * With the app, member-investors send a zero-coin transaction to Fund Manager's Lisa address from their new Lisa address to request membership
    ...
    * If accepted by fund managers, instantiate a new wallet addresses on the A-tier and R-tier chains, associated with this Lisa wallet address and the KYC info the member-investor entered. These are the attributes of the "Member-Investor object." Save the private keys for L-, A-, and R-tiers to the member-investor's mobile device.
...
### Reserve Tier coins must equal at least 34 percent total issuance
